...
async function main() {
  ...
  console.log(`Created sport with id: ${orienteering.id}`);
  const countries = [
  ...
  for (const country of countries) {
    ...
    console.log(`Created country with id: ${countryRecord.countryCode}`);
  }
  const eventName = process.env.EVENT_NAME;
  if (eventName) {
    console.log({ orienteering });
    const dbEventResponse = await prisma.event.findFirst({
      where: {
        name: eventName,
      },
    });
