...
export function registerPublicEventRoutes(router) {
  ...
  const supportedMapsets = new Set([
    "basic",
    "outdoor",
    "winter",
    "aerial",
  ...
  router.get("/map-tiles/:mapset/:tileSize/:z/:x/:y", async c => {
    ...
    if (!supportedMapsets.has(mapset)) {
      return c.json(error("Unsupported mapset", 422), 422);
    }

    if (tileSize !== "256" && tileSize !== "256@2x") {
      return c.json(error("Unsupported tile size", 422), 422);
    }

    if (tileSize === "256@2x" && mapset !== "basic" && mapset !== "outdoor") {
      return c.json(
    ...
    const tileUrl = `https://api.mapy.com/v1/maptiles/${mapset}/${tileSize}/${zoom}/${x}/${y}${search.toString().length ? `?${search.toString()}` : ""}`;
