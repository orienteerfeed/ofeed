...
async function buildEventRouteRequest(
  c: Context<AppBindings>,
  bodyMode: BodyMode,
): Promise<EventRouteRequest> {
...
async function authorizeEventOwnerOrAdmin(
  req: EventRouteRequest,
  res: RouteJsonResponder,
  eventId: string,
  ...
  try {
    const ownership = await ensureEventOwnerOrAdmin(prisma, req.auth, eventId, options);
  ...
  router.post(
    ...
    routeWithValidation(
      ...
      async ({ c, req, res }) => {
        ...
        try {
          await requireEventOwnerOrAdmin(prisma, req.auth, eventId);
      ...
      try {
        ...
        try {
          ...
          logEndpoint(
            req.c,
            'warn',
            'Failed to delete event assets from storage during event deletion',
            {
              eventId,
          ...
          if (classExternalId) {
            try {
              ...
              logEndpoint(
                req.c,
                'error',
                'Class lookup by externalId failed on competitor create',
                {
                  eventId,
                  classExternalId,
                  ...getErrorDetails(error),
        ...
        if (classExternalId) {
          try {
            ...
            logEndpoint(
              req.c,
              'error',
              'Class lookup by externalId failed on external competitor update',
              {
                eventId,
                classExternalId,
                competitorExternalId,
