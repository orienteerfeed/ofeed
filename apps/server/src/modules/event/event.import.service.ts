...
export type OrisEventCandidate = {
  externalEventId: string;
  name: string;
  date?: string;
  ranking: boolean;
...
const ORIS_EVENT_LIST_TIMEOUT_MS = 20_000;
const ORIS_EVENT_LIST_CHUNK_MONTHS = 3;
...
const NAME_KEYS = ['name', 'Name', 'eventName', 'EventName', 'title', 'Title', 'Nazev', 'Název'];
const DATE_KEYS = [
  'date',
  'Date',
  'eventDate',
...
const ORGANIZER_RECORD_TEXT_KEYS = [
  'Name',
  'name',
  'NameCZ',
  'Name_EN',
...
function readStringWithNestedRecordPreference(
  record: GenericRecord,
  keys: readonly string[],
  nestedRecordKeys: readonly string[],
): string | undefined {
  for (const key of keys) {
    const candidate = getCaseInsensitiveValue(record, key);
    if (isRecord(candidate)) {
      for (const nestedKey of nestedRecordKeys) {
        const nestedValue = getCaseInsensitiveValue(candidate, nestedKey);
        const parsed = toStringValue(nestedValue);
        if (parsed) {
          return parsed;
        }
      }
...
function readOrganizer(record: GenericRecord): string | undefined {
  return readStringWithNestedRecordPreference(record, ORGANIZER_KEYS, ORGANIZER_RECORD_TEXT_KEYS);
}
...
function looksLikeEventRecord(record: GenericRecord): boolean {
  ...
  const mentionsEvent =
    keySignature.includes('event') ||
    keySignature.includes('zavod') ||
    keySignature.includes('závod');
...
function buildOrisEventListDateRangeChunks(params: { dateFrom: string; dateTo: string }) {
  const chunks: Array<{ dateFrom: string; dateTo: string }> = [];
  let chunkStart = parseDateOnlyUtc(params.dateFrom);
  const overallEnd = parseDateOnlyUtc(params.dateTo);

  while (chunkStart <= overallEnd) {
    const nextChunkStart = addUtcMonths(chunkStart, ORIS_EVENT_LIST_CHUNK_MONTHS);
    const chunkEndCandidate = addUtcDays(nextChunkStart, -1);
    const chunkEnd = chunkEndCandidate < overallEnd ? chunkEndCandidate : overallEnd;

    chunks.push({
      dateFrom: formatDateOnlyUtc(chunkStart),
      dateTo: formatDateOnlyUtc(chunkEnd),
    });
...
function normalizeSportSource(provider: ExternalEventProvider, value: string): string[] {
  ...
  if (
    ...
    normalized.includes('stafeta') ||
    normalized.includes('štafeta')
  ) {
    keywords.add('relay');
  }
...
function resolveImportedEventDiscipline(
  provider: ExternalEventProvider,
  rawValue?: string,
  ...
  const normalizedUpper = rawValue.trim().toUpperCase();
  ...
  if (normalized.includes('knock') && normalized.includes('sprint')) {
    return 'KNOCKOUT_SPRINT';
  }
  if (
    normalized.includes('night') ||
    normalized.includes('nocni') ||
    normalized.includes('noční')
  ) {
    return 'NIGHT';
  }
  if (normalized.includes('sprint') && normalized.includes('relay')) {
    return 'SPRINT_RELAY';
  }
  if (
    normalized.includes('relay') ||
  ...
  if (
    ...
    normalized.includes('druzstva') ||
    normalized.includes('družstva')
  ) {
    return 'TEAMS';
  }
  if (normalized.includes('ultralong') || normalized.includes('ultra')) {
    return 'ULTRALONG';
  }
  if (
    normalized.includes('middle') ||
    normalized.includes('stredni') ||
    normalized.includes('střední')
  ) {
    return 'MIDDLE';
  }
  if (
    normalized.includes('long') ||
    normalized.includes('dlouha') ||
    normalized.includes('dlouhá')
  ) {
    return 'LONG';
  }
...
export async function loadOrisEventCandidatesByDateRange(params: {
  dateFrom: string;
  dateTo: string;
}): Promise<OrisEventCandidate[]> {
  const deduplicated = new Map<string, OrisEventCandidate>();

  for (const chunk of buildOrisEventListDateRangeChunks(params)) {
    const payload = await fetchExternalPayload(
      buildOrisUrl('getEventList', {
        sport: '1',
        all: '1',
      ...
      {},
      ORIS_EVENT_LIST_TIMEOUT_MS,
    );

    for (const candidate of extractCandidates(payload)) {
      const comparableId = toComparableId(candidate.externalEventId);

      if (!comparableId) {
        continue;
      }

      const mapped: OrisEventCandidate = {
        externalEventId: candidate.externalEventId,
        name: candidate.name,
        date: candidate.date,
      ...
      if (!existing || (!existing.discipline && mapped.discipline)) {
        deduplicated.set(comparableId, mapped);
      }
    }
  }
...
export async function loadExternalEventPreview(
  body: EventImportPreviewBody,
): Promise<ExternalEventPreview> {
  const payload =
    body.provider === 'ORIS'
