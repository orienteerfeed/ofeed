...
const { lexicographicSortSchema, printSchema } = pothosRequire('graphql') as typeof import('graphql');
...
describe('Pothos GraphQL schema', () => {
  it('builds the public root operation fields', () => {
    ...
    expect(sorted(Object.keys(schema.getQueryType()?.getFields() ?? {}))).toEqual(sorted([
      '_empty',
    ...
    expect(sorted(Object.keys(schema.getMutationType()?.getFields() ?? {}))).toEqual(sorted([
      '_empty',
    ...
    expect(sorted(Object.keys(schema.getSubscriptionType()?.getFields() ?? {}))).toEqual(sorted([
      '_empty',
      'competitorUpdated',
